...
#include "lcd.h"  // Use a library for HD44780 I2C
...
int main(void)
{
  ...
  LCD_Send_String(&hi2c1, "Dispenser Ready");
  ...
  while (1)
  {
    if (HAL_GPIO_ReadPin(IR_SENSOR_PORT, IR_SENSOR_PIN) == GPIO_PIN_RESET)
    {
    	 LCD_Clear(&hi2c1);
    	    LCD_Set_Cursor(&hi2c1, 1, 0);
    	    LCD_Send_String(&hi2c1, "Hand Detected");

    	    set_servo_angle(90);  // Dispense
    	    HAL_Delay(1000);
    	    set_servo_angle(0);   // Close

    	    LCD_Clear(&hi2c1);
    	    LCD_Set_Cursor(&hi2c1, 1, 0);
    	    LCD_Send_String(&hi2c1, "Ready Again");
...
void set_servo_angle(uint8_t angle)
{
  // For standard servo: 0.5ms - 2.5ms pulse at 50Hz
  uint16_t pwm = ((angle * 2000) / 180) + 500; // pulse in us
